...
const NavItem = ({ href, children, onClick, index, delay }: NavItemsProps) => {
  return (
    <motion.li
      className="group text-gray-200 font-semibold text-lg"
    ...
    >
      <CLink
        href={href || `/#${children}`}
        className="block p-2 duration-500 hover:text-orange-700 hover:bg-white rounded-2xl"
        onClick={onClick}
        withPadding
      >
        {children}
...
const Navbar = () => {
  ...
  return (
    <motion.header
      ...
      animate="show"
      id="navbar"
      className="fixed inset-x-0 top-0 right-0 z-50 flex items-end justify-between px-8 py-8 duration-500 md:px-6 xl:px-12 bg-orange-700"
    >
      <h1 className="relative text-3xl text-white font-bold group top-1">
        <Link href="/#hero" className="block">
          {/* {author.name} */}
          Vishu Car Transport
          <div className="absolute bottom-1.5 left-0 h-[1px] w-0 duration-300"></div>
        </Link>
      </h1>
